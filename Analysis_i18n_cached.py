...
def cached_load_df_by_exact_name(doc_name: str):
    ...
    Isso faz o botão de idioma rerenderizar a UI sem bater de novo na KB/Noxus.
    """
    if not doc_name:
        return None, None
...
# ✅ Não chama a KB aqui.
# Usa apenas a lista que já veio cacheada da Home, para evitar tela intermediária.
raw_names = st.session_state.get("_kb_names_cache", [])

base_names = sorted(
    set([n.split(" - ")[0].strip() for n in raw_names if n and " - " in n])
)
...
with st.sidebar:
    ...
    st.markdown(f"### {tr('check_another_place')}")
    ...
    with loading:
        with st.spinner(tr("loading_place", place=doc_name)):
            ...
            if df is not None and doc_name == place:
                ...
                df_plot = df.rename(columns={"max_temp": max_temp_label, "min_temp": min_temp_label})
                ...
                fig = px.line(
                    ...
                    markers=True,
                    title=tr("monthly_chart_title"),
                    labels={"value": tr("temperature_label"), "variable": tr("legend_label"), "Date": tr("month_year_label")},
                )
        ...
        if doc_name == place:
            ...
            if hist_df is not None and len(hist_df) > 0:
                ...
                with left:
                    ...
                    components.html(_kpi_card_html(tr("historic_max"), max_temp, max_date, tr("date_label")), height=140)
                    st.write("")
                    components.html(_kpi_card_html(tr("historic_min"), min_temp, min_date, tr("date_label")), height=140)
                ...
                with right:
                    st.markdown(f"""
                        ...
                        <div style="font-size:1.35rem; font-weight:700; color:#ffffff;">
                            {tr("natural_disasters")} <span style="font-weight:500; color:rgba(255,255,255,0.55);"></span>
                        </div>
                        </div>
                    ...
                    if risk_df is not None and len(risk_df) > 0:
                        ...
                        disaster_label_map = {
                            "Seismo": tr("disaster_seismo"),
                            "Hurricane": tr("disaster_hurricane"),
                            "Heatwave": tr("disaster_heatwave"),
                            "Flood": tr("disaster_flood"),
                            "Wildfire": tr("disaster_wildfire"),
                            "Eruption": tr("disaster_eruption"),
                        }

                        plot_df["Tipo"] = plot_df["Tipo"].map(lambda x: disaster_label_map.get(x, x))

                        col_type = tr("type_label")
                        col_events = tr("events_col")
                        col_physical = tr("physical_col")
                        col_human = tr("human_mortality_col")

                        plot_df = plot_df.rename(columns={
                            "Tipo": col_type,
                            "Events": col_events,
                            "Physical": col_physical,
                            "Human (Mortality)": col_human,
                        })

                        max_ev = max(1, plot_df[col_events].max())
                        plot_df[col_events] = (plot_df[col_events] / max_ev)

                        fig_heat = px.imshow(
                            plot_df.set_index(col_type),
                            aspect="auto",
                            text_auto=".0f"
                        )
                        fig_heat.update_layout(
                            ...
                            height=260,
                            coloraxis_colorbar=dict(title=tr("score_label")),
                        )
                        fig_heat.update_xaxes(side="top")
                        st.plotly_chart(fig_heat, use_container_width=True)

                    # ❌ REMOVE daqui:
                    # render_sources_expander(dis_sources, title=tr("disasters_sources"), align="right")

                # ✅ LINHA NOVA: SOURCES ALINHADAS NO MESMO Y
                left_s, right_s = st.columns([0.55, 2.45])

                with left_s:
                    render_sources_expander(hist_sources, title=tr("hist_sources"), align="left")
